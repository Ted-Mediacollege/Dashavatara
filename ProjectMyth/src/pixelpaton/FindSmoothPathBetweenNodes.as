package pixelpaton {
	...
	public class FindSmoothPathBetweenNodes {
		...
		public static function getArrayOfPoints(_nodeArray:Vector.<slashPart>,_evenSpacing:Boolean = false,_dotDistance:Number = 10):Vector.<slashPart> {
			/**
			...
			var pathArray:Vector.<slashPart> = new Vector.<slashPart>();
			...
			while (!stopRender){
				if (percent >= 1) {
					...
					if (drawCurrentItem < _nodeArray.length - 1) {
						var subItem:slashPart = _nodeArray[drawCurrentItem - 1];
						var firstItem:slashPart = _nodeArray[drawCurrentItem];
						...
						if (thirdItem == null) {
							...
							var radians:Number = theSubAngle;
					...
					var _point:slashPart = new slashPart(0, 0, 0);
					if (firstItem!=null && secondItem !=null&& node1 !=null&& node2 !=null){
						...
						_point.t = _nodeArray[drawCurrentItem].t;
		...
		public static function findDistance(_point1:slashPart, _point2:slashPart):Number {
